...
class PodcastReframeEngine(IReframeEngine):
    ...
    def _pipeline(self, video_path: str, output_path: str, autogrid: bool) -> Optional[dict]:
        ...
        # Step 3: Frame-by-Frame Rendering (100% accurate head centering)
        # Processes every frame: detect face → verify center → crop → write
        # Falls back to expression-based panning if frame-by-frame fails
        result = self._render_frame_by_frame(
        ...
        # Fallback: expression-based dynamic panning (if frame-by-frame fails)
        logger.info("podcast_reframe: frame-by-frame failed, falling back to expression panning")
    ...
    # ─── Render: Frame-by-Frame (100% Accurate Centering) ────────────────

    SMOOTHING_FACTOR = 0.15  # Exponential smoothing (0 = no smooth, 1 = instant snap)

    def _render_frame_by_frame(
        self,
        video_path: str,
        output_path: str,
        width: int,
        ...
        """Render by processing every frame — 100% accurate head centering.

        For each frame:
          1. Detect face (MediaPipe) to get current position
          2. Determine which face to follow (from speaker_result)
          3. Compute crop_x so face center is exactly in center of 9:16 crop
          4. Apply exponential smoothing to prevent jitter
          5. Write cropped frame

        After all frames written:
          - Mux original audio (stream-copy) via FFmpeg

        This is slower than expression-based panning but guarantees the face
        is ALWAYS centered, even when the person moves between samples.
        """
        import cv2
        cv2.setNumThreads(0)
        ...
        cap = cv2.VideoCapture(video_path)
        if not cap.isOpened():
            logger.warning("frame_by_frame: cannot open video")
            return None

        total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
        video_fps = cap.get(cv2.CAP_PROP_FPS) or fps

        # Temp video file (no audio)
        temp_video = video_path.rsplit(".", 1)[0] + "_fbf_temp.mp4"

        # Use FOURCC for output — will be re-encoded by FFmpeg after
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        writer = cv2.VideoWriter(temp_video, fourcc, video_fps, (output_w, output_h))

        if not writer.isOpened():
            logger.warning("frame_by_frame: cannot create video writer")
            cap.release()
            return None

        # State for smoothing
        current_crop_x = max_crop_x // 2  # Start at center
        frame_count = 0
        midpoint = width / 2.0

        # Build stable_positions lookup for N-position targeting
        stable_positions = tracked_data.get("stable_positions", {})
        sorted_tracks = sorted(stable_positions.items(), key=lambda kv: kv[1])
        position_to_track = {idx: tid for idx, (tid, _) in enumerate(sorted_tracks)}

        logger.info(
            f"frame_by_frame: starting {total_frames} frames, "
            f"crop_w={crop_w}, output={output_w}x{output_h}"
        )
        ...
        try:
            while True:
                    ...
                    if result.detections:
                        ...
                        for det in result.detections:
                            ...
                            if rel_w < self.MIN_FACE_SIZE_RATIO or rel_w > self.MAX_FACE_SIZE_RATIO:
                                continue
                            scale_to_orig = width / proc_w
                            cx = (bbox.origin_x + bbox.width / 2) * scale_to_orig
                            face_positions.append(cx)

                # --- Determine which face to follow ---
                if face_positions:
                    if len(face_positions) == 1:
                        target_cx = face_positions[0]
                    elif speaker_result and speaker_result.per_frame_speaker:
                        # Find closest speaker assignment
                        frame_idx_approx = frame_count
                        closest_frame = min(
                        ...
                        if closest_frame is not None:
                            ...
                            if target_x_hint is not None:
                                target_cx = min(face_positions, key=lambda x: abs(x - target_x_hint))
                            else:
                                # Fallback: midpoint split
                                if active_speaker == 0:
                                    left_faces = [x for x in face_positions if x < midpoint]
                                    target_cx = np.median(left_faces) if left_faces else np.median(face_positions)
                                else:
                                    right_faces = [x for x in face_positions if x >= midpoint]
                                    target_cx = np.median(right_faces) if right_faces else np.median(face_positions)
                        else:
                            target_cx = min(face_positions, key=lambda x: abs(x - (current_crop_x + crop_w // 2)))
                    else:
                        # No speaker info — follow closest to current position
                        target_cx = min(face_positions, key=lambda x: abs(x - (current_crop_x + crop_w // 2)))

                # --- Compute crop_x to center face ---
                if target_cx is not None:
                    desired_crop_x = int(target_cx - crop_w // 2)
                    desired_crop_x = max(0, min(desired_crop_x, max_crop_x))

                    # Exponential smoothing to prevent jitter
                    current_crop_x = int(
                        current_crop_x + (desired_crop_x - current_crop_x) * self.SMOOTHING_FACTOR
                    )
                    current_crop_x = max(0, min(current_crop_x, max_crop_x))
                # else: hold previous crop_x (no face detected this frame)

                # --- Crop and resize frame ---
                cropped = frame[:, current_crop_x:current_crop_x + crop_w]
                ...
                if frame_count % int(video_fps * 5) == 0:
                    progress_pct = (frame_count / total_frames * 100) if total_frames > 0 else 0
                    logger.info(f"frame_by_frame: {frame_count}/{total_frames} frames ({progress_pct:.0f}%)")

        finally:
            cap.release()
            writer.release()

        if frame_count == 0 or not os.path.exists(temp_video):
            logger.warning("frame_by_frame: no frames written")
            return None

        logger.info(f"frame_by_frame: {frame_count} frames written, muxing audio...")

        # --- Mux original audio onto cropped video ---
        # Re-encode the temp video (mp4v codec is not great) + copy audio
        cmd = [
            "ffmpeg", "-y",
            "-i", temp_video,     # Video from frame-by-frame
            "-i", video_path,     # Original video (for audio)
            "-map", "0:v:0",      # Take video from first input
            "-map", "1:a:0",      # Take audio from second input
            *get_video_encoder_args("medium"),
            "-c:a", "copy",       # Audio stream-copy (zero desync)
            "-movflags", "+faststart",
            "-shortest",
            output_path,
        ]
        ...
        if result.returncode == 0 and os.path.exists(output_path) and os.path.getsize(output_path) > 1000:
            logger.info(f"frame_by_frame: rendering OK ({frame_count} frames)")
            return {
                "output_path": output_path,
                "person_count": tracked_data["person_count"],
                "method": "podcast_frame_by_frame",
                "frames_processed": frame_count,
            }

        if result.stderr:
            logger.warning(f"frame_by_frame: mux failed: {result.stderr[-300:]}")
        return None
