...
class JobRepository(IJobRepository):
    ...
    def _to_entity(self, model: JobModel) -> Job:
        return Job(
            ...
            # Fail-closed: subtypes read from clips_data with False default.
            # Old jobs (pre-fix) that persisted subtype keys with True will
            # keep working; jobs without the key stay OFF.
            broll_image_overlay=bool((model.clips_data or {}).get("broll_image_overlay", False)) if bool(model.broll_enabled) else False,
            broll_behind_person=bool((model.clips_data or {}).get("broll_behind_person", False)) if bool(model.broll_enabled) else False,
            broll_video_footage=bool((model.clips_data or {}).get("broll_video_footage", False)) if bool(model.broll_enabled) else False,
