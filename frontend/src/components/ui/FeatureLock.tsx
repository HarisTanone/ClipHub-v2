 ...
 * Access granted if: superadmin OR is_premium=true.
 * Non-premium users see "Premium" lock animation on click.
 */
export function FeatureLock({ children, featureName, featureCode, isSuperadmin, isPremium, userFeatures = [], className }: FeatureLockProps) {
  const [showNotif, setShowNotif] = useState(false);

  // Access: superadmin OR premium OR (legacy) has feature in list
  const hasAccess = isSuperadmin || isPremium || (featureCode && userFeatures.includes(featureCode));
    ...
    <div className={cn("relative", className)}>
      ...
      {showNotif && (
        <div className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none">
          <div className="animate-[comingSoon_2.5s_ease-out_forwards] flex flex-col items-center gap-3 bg-zinc-900/95 border border-amber-500/30 rounded-2xl px-8 py-6 shadow-2xl shadow-amber-500/10">
            <div className="relative">
              <Sparkles className="h-8 w-8 text-amber-400 animate-pulse" />
              <div className="absolute -top-1 -right-1 w-3 h-3 bg-amber-500 rounded-full animate-ping" />
            </div>
            <div className="text-center">
              <p className="text-sm font-semibold text-zinc-100">Premium Feature</p>
